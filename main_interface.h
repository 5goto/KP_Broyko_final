...
public:
	...
	{
		Init_Favorites_Data_Base favorites_base; //  объекты из файла Init from file
		Init_Manga_Data_Base manga_base;   // они  вернут заполненные из файла базы данных
	...
	auto find_object_in_tree(string name, string reader, int stat)
	{
		Favorites* tmp_struct = new Favorites{ reader, name, stat };
		return favorites_data_base->tree_search(tmp_struct);
	}

	auto find_object_in_table(string title, string genre, int age)
	{
		Manga* tmp_struct = new Manga{ title, genre, age };
		auto operation_result = manga_data_base->search_index(tmp_struct);
	...
	{ 
		...
		vector<Manga*> answer; // это вектор, в которыйь ответы
